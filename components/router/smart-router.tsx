...
export function SmartRouter() {
  ...
  const routes = congested
    ? SIDE_ROUTES.filter((r) => r.type === 'trilha' || r.type === 'cachoeira')
    : SIDE_ROUTES.filter((r) => r.type === 'praia');
  ...
  return (
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 14, fontWeight: '700', color: '#1e293b' }}>🧭 Roteiro Lado B</Text>
        ...
        {routes.map((route) => {
          ...
          return (
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 14, fontWeight: '600', color: '#1e293b' }}>{route.name}</Text>
                <Text style={{ fontSize: 12, color: '#64748b' }}>{route.description}</Text>
                <View style={{ flexDirection: 'row', gap: 10, marginTop: 4 }}>
                  ...
                  {route.difficulty && (
                    ...
                    >
                      <Text style={{ fontSize: 10, color: cfg.color, fontWeight: '600' }}>{route.difficulty}</Text>
                    </View>
                  )}
                </View>
              </View>
